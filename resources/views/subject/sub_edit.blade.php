...
<h1>Edit Subject</h1><br>
	{!! Form::open(array('route'=> ['subject.update',$try->id],'method'=>'PUT', 'class'=>'form-horizontal', 'files'=>true)) !!}
  <div style="height:auto; width:55%; ">

    <div style="text-align: left; font-weight:500;  background-color: #F7DCFA; margin: 0px; " class="row" class="{{ $errors->has('suid') ? 'has-error' : '' }}">
            {{ Form::label('suid', 'Subject ID', array('class'=>'col-md-4 form-group'))}}
            {{ Form::text('suid',$try->suid, array('class'=>'col-md-8 form-control', 'placeholder'=>'Subject ID'))}}
                @if($errors->has('suid'))
    ...
    <div style="text-align: left; font-weight:500;  background-color: #F7DCFA; margin: 0px; margin-top:10px; " class="row" class="{{ $errors->has('fmark') ? 'has-error' : '' }}">
            {{ Form::label('fmark', 'Full Mark', array('class'=>'col-md-4 form-group'))}}
            {{ Form::text('fmark',$try->fmark, array('class'=>'col-md-8 form-control', 'placeholder'=>'Full Mark'))}}
                @if($errors->has('fmark'))
                <div class="col-md-6"></div>
         <span class="col-md-6" style="text-align: right; color:red;">
